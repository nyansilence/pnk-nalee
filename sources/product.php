...
$bread->add(_product,"$com.html");
$assets->addJs("assets/plugins/icheck/icheck.min.js");
$assets->addCss("assets/plugins/icheck/skins/all.css");
$assets->addCss("assets/plugins/icheck/skins/minimal/grey.css");
$assets->addCss("assets/plugins/icheck/skins/minimal/color-scheme.css");

if(isset($_GET['xid'])){
	$xid = addslashes($_GET['xid']);
	$slug = addslashes($_GET['slug']);
	$ar  = array("danhmuc"=>"id_danhmuc","list"=>"id_list","cat"=>"id_cat");
	$find = false;
	foreach($ar as $k=>$v){
		if(!$find){
			$d->query("select id from #_product_$k where id = '$xid' and tenkhongdau='$slug' and hienthi > 0 ");
			if($d->num_rows()){
				$ar[$v] = $xid;
				extract($ar);
...
if($id!='')
{
	...
	$assets->addJs("assets/js/plugins.js");
	$assets->addJs("assets/js/sly.min.js");
	$assets->addJs("assets/plugins/sweetalert/sweetalert2.min.js");
	$assets->addCss("assets/plugins/sweetalert/sweetalert2.min.css");
    #các s?n ph?m khác======================
    ...
    $sql = "select * from #_product where hienthi=1 and id_danhmuc='".$row_detail['id_danhmuc']."' and type='product' and noibat > 0 ";
	...
	if($row_detail['id_danhmuc']){
		...
		$bread->add($rx['ten_'.$lang],'category/'.$rx['tenkhongdau']."-".$rs['id'].'.html');
	}
	...
	if(@$rx & @$r & $row_detail['id_cat']){
		...
		$rs = $d->fetch_array();
		$bread->add($rs['ten_'.$lang],'category/'.$rs['tenkhongdau']."-".$rs['id'].'.html');
	}
	...
	$bread->add($row_detail['ten_'.$lang],getCurrentPageURL());
		...
		$sql="select p.* from ".implode(",",$table)." where 1 ".((count($condition)) ? "  and type='product' and ".implode(" and ",$condition) : '')."  and p.hienthi = 1 ";
		...
		if($id_danhmuc){
			...
			//if(!isAjaxRequest()){
				$xsql = "select ten_$lang,id,noidung_$lang,seo_title,seo_keyword,seo_description from #_product_danhmuc where id='$id_danhmuc' ";
				$d->query($xsql);
		...
		if($id_list){
			...
			//if(!isAjaxRequest()){
				$xsql = "select ten_$lang,id,noidung_$lang,seo_title,seo_keyword,seo_description,id_danhmuc from #_product_list where id='$id_list' ";
				$d->query($xsql);
				$titlex = $d->fetch_array();
				$title_bar = $titlex['ten_'.$lang].' - ';
				$title_cat = $titlex['ten_'.$lang];
				...
				if($titlex['id_danhmuc']){
					...
					$rx = $d->fetch_array();
					$bread->add($rx['ten_'.$lang],'category/'.$rx['tenkhongdau']."-".$rx['id'].'.html');
				}
				$id_list = $titlex['id'];
				$bread->add($title_cat,getCurrentPageURL());
			//}
		...
		if($id_cat){
			...
			//if(!isAjaxRequest()){
				$xsql = "select tenkhongdau,ten_$lang,id,seo_title,seo_keyword,seo_description,id_danhmuc,id_list from #_product_cat where id='$id_cat' ";
				$d->query($xsql);
				$titlex = $d->fetch_array();
				$title_bar = $titlex['ten_'.$lang].' - ';
				$title_cat = $titlex['ten_'.$lang];
				...
				$sql.=" and id_cat = $id_cat";
					...
					$bread->add($title_cat,getCurrentPageURL());
					$d->reset();
					$d->query("select id from #_product_cat where tenkhongdau = '".$titlex['tenkhongdau']."' and type='$source'");
					
					$cat_ids = array();
					foreach($d->result_array() as $k=>$v){
						$cat_ids[] = $v['id'];
					}
					$sql.=  " and id_cat  in (".implode(",",$cat_ids).")";
		...
		if(isAjaxRequest()){
			...
			echo json_encode(array("data"=>$data,"paging"=>$paging,"sql"=>""));
			die;
		}
